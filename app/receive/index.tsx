...
export default function ReceiveScreen() {
  ...
  // QR updates with token, amount, and note (Solana Pay URL when any of those are set)
  const qrValue = (() => {
    if (!walletAddress) return "Connect wallet to show QR";
    const symbol =
      RECEIVE_TOKEN_CHIPS.find((t) => t.id === selectedToken)?.symbol ?? "SOL";
    const hasAmount =
      requestAmount.trim() !== "" && !isNaN(parseFloat(requestAmount));
    const hasNote = note.trim() !== "";
    const useSolanaPay = hasAmount || hasNote || symbol !== "SOL";

    if (!useSolanaPay) {
      // Plain address QR (no params)
      return walletAddress;
    }

    const params: Parameters<typeof buildSolanaPayUrl>[0] = {
      recipient: walletAddress,
    };
    if (hasAmount) {
      // Spec: amount is a human decimal, not lamports
      params.amount = requestAmount.trim();
    }
    if (symbol !== "SOL") {
      const mint = TOKENS[symbol as keyof typeof TOKENS];
      if (mint) params.splToken = mint;
    }
    if (hasNote) {
      params.message = note.trim();
    }
    return buildSolanaPayUrl(params);
  })();
