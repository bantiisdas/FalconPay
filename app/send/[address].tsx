...
export default function SendScreen() {
  ...
  const swapAndSendQuoteTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      const now = Date.now();
      if (now - lastBalanceFetchTimeRef.current < BALANCE_THROTTLE_MS) return;
  ...
  }, [needSwapAndSend, outputAmountAtomic, tokenConfig?.mint, solBalance, wallet]);

  const quote = wallet.quotedata as QuoteResponse | null;
  const requiredSol =
    quote != null ? fromSmallestUnit(quote.inAmount, 9) : 0;
        ...
        [
          {
            ...
            onPress: () => Linking.openURL(`${baseUrl}/${signature}${clusterParam}`),
          },
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      if (isSol) {
        ...
        if (sig) recordSendSol({ signature: sig, toAddress: toAddr, amount: numAmount });
      } else {
        if (!config?.mint) {
